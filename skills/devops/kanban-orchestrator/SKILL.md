...
## Integration with K-Aware Orchestrator

> **Requires:** `k-aware-orchestrator` skill (v1.2+) loaded. Skip this section if the skill is not loaded.

### The Problem

Kanban fan-out spawns N tasks in parallel without knowing if the system can handle it.
If the K-state is Critical or Turbulent, parallel fan-out makes things worse.

### The Solution

**Before Step 3 (fan-out), check the regime:**

```python
from auto_orchestrator import AutoOrchestrator

orch = AutoOrchestrator()
regime_result = orch.before_turn(
    n_active=len(task_graph),
    n_parallel=len(specialist_roster),
    record_tool_call=False,  # ⚠️ don't pollute K-vector with meta-checks
)
action = orch.interpret_actions(regime_result)
```

### Regime → Fan-out Strategy

| Regime | Behavior | Kanban Strategy |
|--------|----------|-----------------|
| **Active** | Normal fan-out | Create all tasks in parallel as usual |
| **Critical** | Serialize | Create tasks one at a time, wait for completion before next |
| **Turbulent** | Pause | Ask user before creating any tasks |
| **Frozen** | Escalate | Don't fan-out — ask user how to proceed |
| **Decayed** | Checkpoint first | Run checkpoint, then resume with reduced parallelism |
| **BYPASS** | Skip check entirely | Zero overhead — proceed with fan-out |

### Serialize Script (Critical Regime)

```python
if regime_result['regime'] == 'Critical':
    for i, (title, assignee, body) in enumerate(task_list):
        # Create one task, wait for it to complete
        task_id = kanban_create(
            title=title, assignee=assignee, body=body,
        )["task_id"]
        # If not the last task, wait for completion
        if i < len(task_list) - 1:
            # The dispatcher will handle serialization —
            # kanban blocks automatically post-dispatch
            print(f"⚠️ K-Aware: Serialized task {i+1}/{len(task_list)}")
    return  # skip normal fan-out
```
